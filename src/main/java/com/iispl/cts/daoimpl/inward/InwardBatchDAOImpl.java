...
public class InwardBatchDAOImpl implements InwardBatchDAO {
	...
	public boolean updateStatus(String batchId, String status) {
		...
		try (Connection connection = DBConnection.getConnection();
			...
			System.out.println(
					"DEBUG: updateStatus updated batch " + batchId + " to " + status + " | Rows affected: " + rows);
			return rows > 0;
	...
	public List<InwardBatch> getBatchesForMicrRepair() {
		...
		String sql = "SELECT ib.inward_batch_id, " + "ib.batch_reference_id, " + "ib.actual_cheque_count, "
				...
				+ "  'MICR_REPAIR_REQUIRED', " + "  'SEND_BACK_TO_MAKER_MICR', " + "  'MICR_REPAIR_COMPLETED', "
				+ "  'REJECTION_REQUESTED')) " + "AS micr_repair_pending_count "

				+ "FROM inward_batch ib "

				+ "WHERE EXISTS ( " + "    SELECT 1 " + "    FROM inward_cheque ic "
				+ "    WHERE ic.inward_batch_id = ib.inward_batch_id " + "    AND ic.cheque_status IN "
				+ "    ('MICR_REPAIR_PENDING', " + "     'MICR_REPAIR_IN_PROGRESS', " + "     'MICR_REPAIR_REQUIRED', "
	...
	public List<InwardReportChequeDTO> getChequesByBatch() {
		...
		String sql = "SELECT " + "ic.inward_cheque_id, " + "ic.inward_batch_id, " + "ic.cheque_number, "
				+ "ic.micr_code, " + "ic.drawee_name, " + "ic.drawee_account_number, " + "ic.payee_name, "
				+ "ic.payee_account_number, " + "ic.cheque_amount, " + "ic.cheque_date, " + "ic.cheque_status, "
				+ "drawee_bank.bank_name AS drawee_bank, " + "presenting_bank.bank_name AS presenting_bank, "
				+ "r.rejection_id, " + "r.rejected_reason_id, " + "r.remarks, " + "r.rejected_by, " + "r.rejected_at, "
				+ "rr.rejected_reason_code, " + "rr.rejected_reason_name, " + "rr.rejected_reason_description "
				+ "FROM inward_cheque ic " + "INNER JOIN inward_batch ib "
				+ "ON ib.inward_batch_id = ic.inward_batch_id " + "LEFT JOIN master_account_new drawee_acc "
				+ "ON drawee_acc.account_number = ic.drawee_account_number " + "LEFT JOIN master_branch drawee_branch "
				+ "ON drawee_branch.branch_id = drawee_acc.branch_id " + "LEFT JOIN master_bank drawee_bank "
				+ "ON drawee_bank.bank_code = drawee_branch.bank_code " + "LEFT JOIN master_account_new payee_acc "
				+ "ON payee_acc.account_number = ic.payee_account_number " + "LEFT JOIN master_branch payee_branch "
				+ "ON payee_branch.branch_id = payee_acc.branch_id " + "LEFT JOIN master_bank presenting_bank "
				+ "ON presenting_bank.bank_code = payee_branch.bank_code " + "LEFT JOIN inward_cheque_rejection r "

...
public class OutwardBatchDAOImpl implements OutwardBatchDAO {
	...
	public List<OutwardBatch> getBatchesReadyForDataEntry() {
		...
		// Batches remain visible in Data Entry until Maker explicitly submits to
		// Checker
		String sql = "SELECT ob.outward_batch_id, ob.batch_reference_id, ob.actual_cheque_count, "
				+ "ob.actual_total_amount, ob.batch_status, ob.uploaded_by, ob.uploaded_at " + "FROM outward_batch ob "
				+ "WHERE UPPER(TRIM(ob.batch_status)) IN ('PENDING_DATA_ENTRY', 'ON_HOLD', 'PENDING_MAKER_PROCESS') "
	...
	public List<OutwardBatch> getScanBatchesReadyForDataEntry() {
		...
		String sql = "SELECT sb.scanned_batch_id AS outward_batch_id, sb.batch_reference_id, "
				+ "sb.actual_cheque_count, sb.actual_total_amount, sb.batch_status, sb.uploaded_by, sb.uploaded_at "
				+ "FROM scan_batch sb " + "WHERE UPPER(TRIM(sb.batch_status)) = 'PENDING_MAKER_PROCESS' "
				+ "AND sb.actual_cheque_count > 0 "
				+ "AND NOT EXISTS (SELECT 1 FROM outward_batch ob WHERE ob.outward_batch_id = sb.scanned_batch_id) "
				+ "ORDER BY sb.uploaded_at ASC";
	...
	public String createOutwardBatchFromScan(Connection connection, String scannedBatchId) {
		...
		String insertSql = "INSERT INTO outward_batch "
				+ "(outward_batch_id, batch_reference_id, actual_cheque_count, actual_total_amount, batch_status, uploaded_by) "
				+ "SELECT scanned_batch_id, batch_reference_id, actual_cheque_count, actual_total_amount, "
	...
	public String getOutwardBatchIdByScannedBatchId(String scannedBatchId) {
		...
		String sql = "SELECT outward_batch_id FROM outward_batch WHERE outward_batch_id = ?";
		try (Connection connection = DBConnection.getConnection();
				PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
	...
	public String getScannedBatchIdByOutwardBatchId(String outwardBatchId) {
		...
		String sql = "SELECT sb.scanned_batch_id FROM scan_batch sb "
				+ "INNER JOIN outward_batch ob ON sb.scanned_batch_id = ob.outward_batch_id "
				+ "WHERE ob.outward_batch_id = ?";
	...
	public boolean updateOutWardBatchStatus(String outwardBatchId, String batchStatus) {
		try (Connection connection = DBConnection.getConnection()) {
			return updateOutWardBatchStatus(connection, outwardBatchId, batchStatus);
		} catch (SQLException exception) {
	...
	public void updateBatchStatus(String batchId, String status) {
		updateOutWardBatchStatus(batchId, status);
	}
	...
	public List<OutwardBatch> searchBatches(String batchId, String status) {
		...
		StringBuilder sql = new StringBuilder("SELECT outward_batch_id, batch_reference_id, actual_cheque_count, "
				+ "actual_total_amount, batch_status, uploaded_by, uploaded_at FROM outward_batch WHERE 1 = 1 ");
		List<String> parameters = new ArrayList<>();

		if (batchId != null && !batchId.trim().isEmpty()) {
			sql.append("AND (LOWER(outward_batch_id) LIKE LOWER(?) OR LOWER(batch_reference_id) LIKE LOWER(?)) ");
			String search = "%" + batchId.trim() + "%";
		...
		try (Connection connection = DBConnection.getConnection();
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					batchList.add(mapOutwardBatch(rs));
				}
			}
		} catch (SQLException exception) {
	...
	public List<OutwardBatch> getPendingBatches(int pageNumber, int pageSize) {
		...
		try (Connection connection = DBConnection.getConnection();
			...
			ps.setString(1, "PENDING_CHECKER_PROCESS");
			ps.setString(2, "ON_HOLD");
	...
	public List<OutwardBatch> getRecentBatches() {
		List<OutwardBatch> batchList = new ArrayList<>();
		String sql = "SELECT outward_batch_id, batch_reference_id, actual_cheque_count, actual_total_amount, "
				+ "batch_status, uploaded_by, uploaded_at FROM outward_batch ORDER BY uploaded_at DESC LIMIT 20";
	...
	public List<MicrRepairBatch> getOutwardMicrRepairBatches() {
		...
		String sql = "SELECT ob.outward_batch_id, ob.uploaded_at, ob.actual_cheque_count, ob.batch_status, "
				+ "COUNT(oc.outward_cheque_id) AS micr_errors FROM outward_batch ob "
				+ "JOIN outward_cheque oc ON oc.outward_batch_id = ob.outward_batch_id "
				+ "WHERE UPPER(TRIM(oc.cheque_status)) = 'PENDING_MICR_REPAIR' "
				+ "GROUP BY ob.outward_batch_id, ob.uploaded_at, ob.actual_cheque_count, ob.batch_status "
		...
		try (Connection connection = DBConnection.getConnection();
			...
			while (rs.next()) {
				...
				batch.setBatchId(rs.getString("outward_batch_id"));
				batch.setScanDate(rs.getTimestamp("uploaded_at"));
				batch.setTotalCheques(rs.getInt("actual_cheque_count"));
				batch.setMicrErrors(rs.getInt("micr_errors"));

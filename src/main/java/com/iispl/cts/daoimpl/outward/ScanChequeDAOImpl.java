...
public class ScanChequeDAOImpl implements ScanChequeDAO {
	...
	public String saveBatch(Connection connection, List<ScanCheque> chequeList) {
		...
		String insertSql = "INSERT INTO scan_cheque ("
		...
		String updateSql = "UPDATE scan_cheque SET " + "scanned_batch_id = ?, cheque_number = ?, micr_code = ?, "
				+ "drawee_name = ?, drawee_account_number = ?, payee_name = ?, "
				+ "payee_account_number = ?, cheque_amount = ?, cheque_date = ?, "
				+ "cheque_status = ?, account_id = ?, created_at = ?, city_code = ?, "
				+ "bank_code = ?, branch_code = ?, cheque_image_front = ?, "
				+ "cheque_image_back = ? WHERE scanned_cheque_id = ?";

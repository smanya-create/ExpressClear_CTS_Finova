...
public class InwardDataEntryController extends GenericForwardComposer<Component> {
    ...
    private int findFirstPendingIndex() {
        if (activeQueue == null || activeQueue.isEmpty()) return 0;
    ...
    private void displayCurrentCheque() {
        ...
        if (lblDataStatus != null) {
            ...
            if (isSentBackStatus(status)) {
                ...
                lblDataStatus.setStyle("background-color: #fee2e2; color: #dc2626; border: 1px solid #fca5a5; font-weight: 700;");
            } else if (InwardChequeStatus.REJECTION_REQUESTED.name().equalsIgnoreCase(status)) {
                lblDataStatus.setValue("REJECT REQ");
                lblDataStatus.setStyle("background-color: #fef2f2; color: #b91c1c; border: 1px solid #fecaca; font-weight: 700;");
                ...
                lblDataStatus.setStyle("background-color: #e0f2fe; color: #0284c7; border: 1px solid #bae6fd; font-weight: 700;");
            } else if (InwardChequeStatus.CHECKER_PROCESSING_PENDING.name().equalsIgnoreCase(status)
                    || InwardChequeStatus.COMPLETED.name().equalsIgnoreCase(status)
                    || "ACCEPTED".equalsIgnoreCase(status)
                    || "DATA_ENTRY_COMPLETED".equalsIgnoreCase(status)) {
                lblDataStatus.setValue("COMPLETED");
                lblDataStatus.setStyle("background-color: #dcfce7; color: #16a34a; border: 1px solid #bbf7d0; font-weight: 700;");
            } else {
                lblDataStatus.setValue("PENDING");
                lblDataStatus.setStyle("background-color: #fef3c7; color: #d97706; border: 1px solid #fde68a; font-weight: 700;");
            }
        }
        ...
        if (txtAmountInWords != null) txtAmountInWords.setValue(convertToIndianCurrencyWords(item.getChequeAmount()));
    ...
    private void updateProgressBar() {
        ...
        if (pmBatchProgress != null) pmBatchProgress.setValue(percentage);
        if (lblProgressText != null) lblProgressText.setValue(resolvedCount + "/" + total + " (" + percentage + "%)");
    ...
    private void updateNavigationState() {
        if (btnPrevCheque != null) btnPrevCheque.setDisabled(activeQueue == null || currentIndex <= 0);
        if (btnNextCheque != null) btnNextCheque.setDisabled(activeQueue == null || currentIndex >= activeQueue.size() - 1);
    }
    ...
    private void saveRejectionRequestRecord(String chequeId, String batchId, String reasonId, String remarks, String userId, String stage) {
    ...
    public void onClick$btnSubmitToChecker() {
        ...
        long rejected = activeQueue.stream().filter(c -> InwardChequeStatus.REJECTION_REQUESTED.name().equalsIgnoreCase(c.getChequeStatus())
                || InwardChequeStatus.REJECTED.name().equalsIgnoreCase(c.getChequeStatus())).count();
    ...
    private static String convertNumberToWords(long n) {
        ...
        if ((n / 10000000) > 0) { sb.append(convertNumberToWords(n / 10000000)).append(" Crore "); n %= 10000000; }
        if ((n / 100000) > 0) { sb.append(convertNumberToWords(n / 100000)).append(" Lakh "); n %= 100000; }
        if ((n / 1000) > 0) { sb.append(convertNumberToWords(n / 1000)).append(" Thousand "); n %= 1000; }
        if ((n / 100) > 0) { sb.append(convertNumberToWords(n / 100)).append(" Hundred "); n %= 100; }
        if (n > 0) {
            if (n < 20) {
                sb.append(UNITS[(int) n]).append(" ");
            } else {
                sb.append(TENS[(int) (n / 10)]).append(" ");
                if ((n % 10) > 0) sb.append(UNITS[(int) (n % 10)]).append(" ");
            }
        }
